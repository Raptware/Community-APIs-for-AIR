...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://sf.net/saxon" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ifn="urn:internal:functions"
	...
	<xsl:template name="description">
		...
		<!-- BBOS - versioning transform -->
		
		<xsl:variable name="bbCustomsText">
			<xsl:value-of select="prolog/asCustoms/bbosversion"/>
		</xsl:variable>
		<xsl:choose>
		<xsl:when test="self::apiClassifier">
				<xsl:if test="string-length($bbCustomsText) &gt; 0">
					<br/><br/>
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td style="white-space:nowrap" valign="top">
								<b>BlackBerry 10 Version:<xsl:value-of select="$nbsp"/></b>
							</td>
							<td>
								<xsl:value-of select="$bbCustomsText"/>
							</td>
						</tr>
					</table>
				</xsl:if>
